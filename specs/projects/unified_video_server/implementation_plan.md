...
- [ ] **Phase 2: Homepage + video card + cold-start.** Implement `top_bar.ts`, `video_card.ts`, `empty_state.ts`, `homepage.ts`. Full visual design from `ui_design.md`. Top bar with home link in video view. End state: navigating to `/` shows the polished homepage; cards link into video view; empty `videos/` shows cold-start panel.
