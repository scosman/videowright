...
- **Export to MP4.** Two export commands: `videowright record` (screenshot-based capture via Playwright, auto-advances using each segment's `advances` array) and `videowright render` (deterministic CDP-driven frame export with controlled clock -- frames are byte-identical across runs). Both require ffmpeg on PATH and Playwright installed. Each segment declares `advances: [t1, t2, ...]` (seconds) that drive render/record timing. The render command puppets the browser's animation clock so Three.js, CSS animations, WAAPI, and everything else renders frame-perfect.
