...
- **Timing is `ctx.waitForNext()` and `ctx.hold(ms)` only.** Never use `setTimeout` or `setInterval` in segments — they break render-mode clock control. Use `ctx.waitForNext()` for interactive beats and `ctx.hold(ms)` for timed pauses.
