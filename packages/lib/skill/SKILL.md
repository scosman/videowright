...
- **Timing is `ctx.waitForNext()` and `ctx.hold(ms)`.** Prefer `ctx.waitForNext()` for interactive beats and `ctx.hold(ms)` for control-flow pauses. `setTimeout`/`setInterval` are virtualized by the render shim and work deterministically, but `ctx.hold` integrates with the abort signal and is idiomatic. Use WAAPI or CSS animations for smooth visual motion.
