...
Videowright has two export commands: `record` and `render`. Both produce an MP4 file using Playwright (headless Chromium) for screenshot capture and ffmpeg for encoding.

## `videowright record`

Screenshot-based capture in interactive mode. The player runs normally; the driver fires `triggerNext()` at times specified by each segment's `advances` array.
...
1. Boots the dev server internally (auto-assigned port, HUD hidden).
2. Launches headless Chromium via Playwright.
3. Navigates to the player and waits for it to be ready.
4. Validates all segments' `advances` arrays.
5. For each segment, captures screenshots at the configured fps and fires `triggerNext()` at each advance time.
6. Pipes screenshots to ffmpeg for MP4 encoding.

### Characteristics

- **Mode**: interactive (segments use wall-clock timing for `hold`).
- **Default fps**: 30.
- **Determinism**: Not fully deterministic — wall-clock timing can vary slightly between runs.
- **Best for**: Quick previews, videos where exact frame reproducibility is not needed.
...
| `--fps <n>` | `30` (record) / `60` (render) | Frames per second. |
...
Both commands accept an optional positional argument for the timeline path. Without it, they use the same default discovery as `videowright dev` (most recently modified `timeline.ts` under `videos/`).

## Dependencies

### ffmpeg

Both commands require `ffmpeg` installed and available on `PATH`. The CLI auto-detects it. If ffmpeg is not found, the command errors with a clear message.
...
Both commands use Playwright to launch headless Chromium. Playwright is a dev dependency of Videowright. If Chromium is not installed, Playwright will prompt to install it:
...
The output is an H.264-encoded MP4 with `yuv420p` pixel format (widely compatible). Audio is not included — videos are always silent in the export.

## Advances validation

Both commands validate every segment's `advances` array before starting capture:
...
During recording/rendering, the driver checks for coherence issues:
...
Exported videos are always silent. Audio playback is not implemented in Videowright. If the video has voiceover intent, the VO script exists for review and external TTS/mixing in post — it is not rendered into the MP4.
...
| Export takes a long time | `render` at 60fps captures every frame individually. A 30-second video = 1800 frames. Use `--verbose` to see progress. For quick previews, use `record` at lower fps. |
...
| Output file already exists | Both commands overwrite the output file (`-y` flag to ffmpeg). |
